...
# lyrs <- list.files(clim, pattern = "bio_", full.names = T) %>% gtools::mixedsort() %>% raster::stack(.)
#
# srad <- list.files(clim, pattern = "srad", full.names = T) %>% gtools::mixedsort() %>% raster::stack(.)
...
outD <- choose.dir()
if(!file.exists(paste0(outD, "/speciesOccurenceAndClimateData.csv"))){
  tbls <- read.csv(paste0(outD, "/speciesOccurenceData.csv"))
  tbls <- cbind(tbls, raster::extract(x = lyrs, y = tbls[,c("longitude", "latitude")]))
  tbls <- cbind(tbls, data.frame(Altitude = raster::extract(x = altitude, y = tbls[,c("longitude", "latitude")])))
  write.csv(tbls, paste0(outD, "/speciesOccurenceAndClimateData"), row.names = F)
} else {
  tbls <- read.csv(paste0(outD, "/speciesOccurenceAndClimateData.csv"))
}
...
tbls <- tbls %>% dplyr::select(Taxon_final:bio_19, srad, wind, vapr, Altitude)
levels(tbls$Taxon_final) <- gsub("SpeciesGenucs", ". ", levels(tbls$Taxon_final))
levels(tbls$Taxon_final) <- gsub("_", " ", levels(tbls$Taxon_final))
my_order <- rev(levels(tbls$Taxon_final))

# Descriptive statistics
psych::describe(tbls %>% dplyr::select(bio_1:Altitude)) %>% write.csv(., file = "../_processed/speciesGenus/descriptive_stats.csv", row.names = T)
...
g <- tbls %>%
  dplyr::select(Taxon_final, latitude, longitude, bio_1:Altitude) %>%
  tidyr::gather(., key = "Variable", value = "Value", -(Taxon_final:longitude)) %>%
  ggplot2::ggplot(aes(x = factor(Taxon_final, my_order), y = Value, colour = Taxon_final)) +
...
ggsave(filename = "../_processed/speciesGenus/individual_boxplots.png", plot = g, units = "in", width = 15, height = 10)
...
lapply(1:length(varList), function(i){
  ...
  ggsave(filename = paste0("../_processed/speciesGenus/jitter_plots/", varList[i], ".png"), plot = g, units = "in", width = 10, height = 8)
  rm(g, variable)
})

# Removing NULL records
tbls$id <- 1:nrow(tbls)
df <- tbls %>% dplyr::select(id, longitude, latitude, bio_1:Altitude) %>% tidyr::drop_na()
...
pca1 <- df %>% dplyr::select(longitude:Altitude) %>% FactoMineR::PCA(scale.unit = T, graph = F)
pca1$ind$coord %>%
  data.frame %>%
  mutate(Species = tbls$taxon_final[intersect(rownames(df), tbls$id)]) %>%
...
fviz_screeplot(pca1, addlabels = TRUE, ylim = c(0, 50)) %>%
  ggsave(filename = "../_processed/pca_climate_coords_altitude/eigen_vals.png", plot = ., units = "in", width = 10, height = 8)

# Control variable colors using their contributions
fviz_pca_var(pca1, col.var = "cos2", # "contrib"
             gradient.cols = c("#00AFBB", "#E7B800", "#FC4E07"),
             repel = TRUE) %>%
  ggsave(filename = "../_processed/pca_climate_coords_altitude/cos2.png", plot = ., units = "in", width = 10, height = 8)
...
fviz_contrib(pca1, choice = "var", axes = 2, top = 20) %>%
  ggsave(filename = "../_processed/pca_climate_coords_altitude/contribution_axis2.png", plot = ., units = "in", width = 10, height = 8)
...
fviz_pca_biplot(pca1,
                label = "var",
                habillage = tbls$taxon_final[intersect(rownames(df), tbls$id)],
                addEllipses = TRUE,
                ellipse.level = 0.95) %>%
  ggsave(filename = "../_processed/pca_climate_coords_altitude/biplot_species.png", plot = ., units = "in", width = 10, height = 8)
...
fviz_pca_biplot(pca1,
                ...
                addEllipses = TRUE,
                ellipse.level = 0.95) %>%
  ggsave(filename = "../_processed/pca_climate_coords_altitude/biplot_type.png", plot = ., units = "in", width = 10, height = 8)
...
fviz_pca_ind(pca1,
  ...
  ggsave(filename = "../_processed/species.png", plot = ., units = "in", width = 10, height = 8)
...
ggsave(filename = "../_processed/pca_climate_coords_altitude/interpretation_cluster3.png", plot = g, units = "in", width = 8, height = 10)
...
g <- fviz_cluster(clust_pca, geom = "point", main = "Factor map") +
  ...
  geom_vline(xintercept = 0) +
  theme_minimal()
ggsave(filename = "../_processed/pca_climate_coords_altitude/clustering.png", plot = g, units = "in", width = 10, height = 8)
...
g <- cross_tab %>% ggplot(aes(x = factor(Cluster), y = Species, fill = Freq)) +
  ...
  theme(axis.text = element_text(size = 15),
        ...
        legend.title = element_text(size = 15, face = "bold"),
        legend.text =  element_text(size = 15))
ggsave(filename = "../_processed/pca_climate_coords_altitude/species_by_cluster.png", plot = g, units = "in", width = 8, height = 10)
